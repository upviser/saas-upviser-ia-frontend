...
export const Plans: React.FC<Props> = ({ content, services, index, payment, step, style, forms, integrations, domain }) => {
  ...
  return (
    <>
      <PopupPlans popup={popup} setPopup={setPopup} plan={plan} services={services} payment={payment} content={content} step={step} style={style} typePrice={typePrice} forms={forms} integrations={integrations} domain={domain} />
                ...
                <div className='flex gap-6 justify-around flex-wrap'>
                  {
                    services?.find(service => service._id === content.service?.service)?.plans?.plans.map((plan, index) => (
                      <div className={`${index === 0 ? plan1Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 1 ? plan2Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 2 ? plan3Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 3 ? plan4Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 4 ? plan5Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : ''} transition-all duration-500 p-6 flex flex-col gap-4 w-full max-w-96 justify-between`} key={plan._id} style={{ boxShadow: style.design === 'Sombreado' ? `0px 3px 20px 3px ${style.borderColor}` : '', borderRadius: style.form === 'Redondeadas' ? `${style.borderBlock}px` : '', border: style.design === 'Borde' ? `${plan.recommended ? 2 : 1}px solid ${plan.recommended ? style.primary : style.borderColor}` : '', backgroundColor: content.info.image }}>
                        <div className='flex flex-col gap-4'>
                          {
                            plan.recommended
                          ...
                          <div className='flex gap-2 w-fit m-auto'>
                            ...
                            <p ref={index === 0 ? plan1Ref : index === 1 ? plan2Ref : index === 2 ? plan3Ref : index === 3 ? plan4Ref : index === 4 ? plan5Ref : ''} className='my-auto'>Mes</p>
                          </div>
                          {
