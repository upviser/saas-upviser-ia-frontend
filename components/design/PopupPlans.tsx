...
export const PopupPlans: React.FC<Props> = ({ popup, setPopup, plan, services, payment, content, step, style, typePrice, forms, integrations, domain }) => {
  ...
  return (
    <div className={`${popup.view} ${popup.opacity} transition-opacity duration-200 w-full h-full top-0 fixed bg-black/30 flex z-50 px-4`}>
      <div ref={popupRef} onMouseEnter={() => setPopup({ ...popup, mouse: true })} onMouseLeave={() => setPopup({ ...popup, mouse: false })} className={`${popup.opacity === 'opacity-1' ? 'scale-1' : 'scale-90'} max-w-[600px] transition-transform duration-200 w-full max-h-[600px] overflow-y-auto m-auto flex flex-col`} style={{ boxShadow: '0px 3px 20px 3px #11111120', background: content.info.image && content.info.image !== '' ? content.info.image : '#ffffff', color: content.info.textColor && content.info.textColor !== '' ? content.info.textColor : '#111111', borderRadius: style.form === 'Redondeadas' ? `${style.borderBlock}px` : '' }}>
        {
                          ...
                          <>
                            ...
                            </div>
                            {
                              content.info.video === 'Realizar pago'
                                ...
                                : <div className='flex flex-col gap-6 pb-6 px-6 md:pb-8 md:px-8 w-full'>
                                  {
                                    error !== ''
                                      ? <p className='px-2 py-1 bg-red-500 text-white w-fit'>{error}</p>
                                      : ''
