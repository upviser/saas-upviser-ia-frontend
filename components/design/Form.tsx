            ...
            ? (
              <div className={`w-full flex`}>
                <form ref={formRef} className={`${formLoaded ? 'opacity-1' : 'opacity-0 translate-y-6'} transition-all duration-500 flex w-full`} onSubmit={async (e: any) => {
                  ...
                  <div className={`flex flex-col gap-4 h-fit m-auto w-full p-6 md:p-8 max-w-[500px]`} style={{ boxShadow: style.design === 'Sombreado' ? `0px 3px 20px 3px ${style.borderColor}` : '', borderRadius: style.form === 'Redondeadas' ? `${style.borderBlock}px` : '', border: style.design === 'Borde' ? `1px solid ${style.borderColor}` : '', backgroundColor: content.info.image && content.info.image !== '' ? content.info.image : '', color: content.info.textColor }}>
                    {
                      message !== ''
                        ? (
                          <>
