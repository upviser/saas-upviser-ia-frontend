...
export async function generateMetadata() {
  ...
  return {
    ...
    openGraph: {
      title: home?.metaTitle && home?.metaTitle !== '' ? home?.metaTitle : '',
      description: home?.metaDescription && home?.metaDescription !== '' ? home?.metaDescription : '',
      url: domain.domain === 'upviser.cl' ? `${process.env.NEXT_PUBLIC_WEB_URL}/` : `https://${domain.domain}/`,
    ...
    <div className="flex flex-col">
      {
        design.pages?.map((page: any) => {
          if (page.page === 'Inicio') {
            return (
              <div key={page._id} className="flex flex-col" style={{ background: page.backgroundType === 'Color' ? page.bgColor : page.backgroundType === 'Degradado' ? `${page.bgType === 'Lineal' ? 'linear' : 'radial'}-gradient(${page.bgType === 'Lineal' ? `${page.bgAngle}deg` : 'circle'}, ${page.bgColor1}, ${page.bgColor2})` : `url("${page.bgImage}")`, backgroundSize: 'cover' }}>
                {
                  page.design.map((content: any, index: any) => {
                    if (content.content === 'Carrusel') {
                      return <Slider key={content.content} info={content.info} index={index} calls={calls} forms={forms} design={design} payment={payment} style={style} storeData={storeData} domain={domain} tenantId={tenantId} page={page} />
                    } else if (content.content === 'Bloque 1') {
                      return <Block1 key={content.content} content={content} index={index} calls={calls} forms={forms} design={design} payment={payment} style={style} storeData={storeData} domain={domain} tenantId={tenantId} page={page} />
                    } else if (content.content === 'Bloque 2') {
                      return <Block2 key={content.content} content={content} index={index} calls={calls} forms={forms} design={design} payment={payment} style={style} storeData={storeData} domain={domain} tenantId={tenantId} page={page} />
                    } else if (content.content === 'Bloque 3') {
                      return <Block3 key={content.content} content={content} index={index} calls={calls} forms={forms} design={design} payment={payment} style={style} storeData={storeData} domain={domain} tenantId={tenantId} page={page} />
                    } else if (content.content === 'Bloque 4') {
                      return <Block4 key={content.content} content={content} index={index} calls={calls} forms={forms} design={design} payment={payment} style={style} storeData={storeData} domain={domain} tenantId={tenantId} page={page} />
                    } else if (content.content === 'Bloque 5') {
                      return <Block5 key={content.content} content={content} index={index} calls={calls} forms={forms} design={design} payment={payment} style={style} storeData={storeData} domain={domain} tenantId={tenantId} page={page} />
