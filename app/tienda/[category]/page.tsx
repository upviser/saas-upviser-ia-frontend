...
async function fetchCategory (category: string, tenantId: string) {
...
async function fetchProductsCategory (category: string, tenantId: string) {
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      url: `${domain.domain === 'upviser.cl' ? process.env.NEXT_PUBLIC_WEB_URL : `https://${domain.domain}`}/tienda/${category.slug}`
    }
  }
}
...
export default async function CategoryPage({ params }: { params: { category: string } }) {
  ...
  const category = await fetchCategory(params.category, tenantId)

  const categoriesData = fetchCategories(tenantId)

  const productsCategoryData = fetchProductsCategory(category.category, tenantId)
  ...
  return (
    <div className="flex flex-col" style={{ backgroundColor: design?.categoryPage[0].bgColor, color: design?.categoryPage[0].textColor }}>
      {
        design?.categoryPage[0].design?.map((content: any, index: any) => {
          if (content.content === 'Carrusel') {
            ...
            return <Subscribe key={content.content} info={ content.info } style={style} tenantId={tenantId} page={design?.categoryPage[0]} />
