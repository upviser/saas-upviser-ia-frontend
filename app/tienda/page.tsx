...
export async function generateMetadata() {
  ...
  return {
    ...
    openGraph: {
      title: home?.metaTitle && home?.metaTitle !== '' ? home?.metaTitle : '',
      description: home?.metaDescription && home?.metaDescription !== '' ? home?.metaDescription : '',
      url: domain.domain === 'upviser.cl' ? `${process.env.NEXT_PUBLIC_WEB_URL}/tienda` : `https://${domain.domain}/tienda`
    }
  }
}
...
export default async function ShopPage() {
  ...
  const [design, products, categories, calls, forms, services, storeData, payment, style, integrations, domain] = await Promise.all([designData, productsData, categoriesData, callsData, formsData, servicesData, storeDataData, paymentData, styleData, integrationsData, domainData])
